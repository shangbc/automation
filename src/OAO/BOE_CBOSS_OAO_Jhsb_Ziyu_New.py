    ...
    try:
        ...
        for i in range(1, batchnumLis + 1):
            ...
            try:
                ...
                updateSqlExt1NotNull = """
                                        update [:regionCode]ord_interate_subord_sync_[:region] a
                                        set a.state = 'DF',
                                            a.active_flag = '2',
                                            a.n_ext_1 = a.n_ext_1+1
                                        where done_date >trunc(sysdate)
                                              and done_date<sysdate-1/1440
                                              and error_msg like 'H04-激活不成功%'
                                              and error_msg != 'H04-激活不成功,证件号码被预占'
                                              ...
                                              and order_id in [:orderId]
                                        """
                ora.sqlExecute(vsoDb, updateSqlExt1NotNull, dictValues)
                logging.info("数据修复成功！")
        ...
        if repairNum != 0:
            try:
                # 记录相关的治愈信息
                dbpRptInsertSql = "insert into bomc.RPT_GW_AUTOFIX values ('CBOSS-OAO激活失败自动修改为DF','陈斌',trunc(sysdate)," + \
